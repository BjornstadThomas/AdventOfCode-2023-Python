...
def count_matches(winning_numbers, player_numbers):
    ...
    return sum(num in winning_numbers for num in player_numbers)


def process_scratchcards(cards):
    """Process all scratchcards and return the total count including won cards."""
    total_cards = len(cards)  #Start with original number of cards
    won_cards = [0] * len(cards)  #Track won cards
